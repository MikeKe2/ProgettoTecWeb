...
var Stories = new Vue({
    ...
    methods: {
            ...
            $.get("/allStories", function (Serverdata) {
                for (let i = 0; i < Serverdata.length; i++) {
                    Stories.list.push({
                        ...
                        visibility: Serverdata[i].visibility
                    });
                }
            });
        },
            ...
            if (newname != null && newname != "") {
                $.ajax({
                    ...
                    data: {
                        name: this.list[index].name,
                        newName: newname,
                        visibility: this.list[index].visibility
                    },
                    success: function () {
        ...
        getQR: function (index) {
            ...
            qrmaker.makeCode(encodeURI("https://site181993.tw.cs.unibo.it/avventura/" + $('#user').html() + "/" + this.list[index].name)); // make another code.
            $(".modal").modal();
        },
